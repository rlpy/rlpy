...
def main(jobID=-1,              # Used as an indicator for each run of the algorithm
    ...
    iFDDOnlineThreshold         = 500 
    BatchDiscoveryThreshold     = 500 if not 'BatchDiscoveryThreshold' in globals() else BatchDiscoveryThreshold  # Minimum relevance required for representation expansion techniques to add a feature 
    ...
    representation.discovery_threshold = 0.0001 # small number, all feature below should be added
    for featInd in arange(19):
	    representation.inspectPair(featInd,featInd+1,1) # try changing last param, td_error, if not work
    representation.inspectPair(0,19,1)
    representation.discovery_threshold = iFDDOnlineThreshold
    representation.showFeatures()
