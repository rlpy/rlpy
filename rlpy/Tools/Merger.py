...
def readMatrixFromFile(filename):
    # Reads a matrix in a text file specified by filename into a numpy matrix
    # and returns it
    x = loadtxt(filename)
    if len(x.shape) == 1:
...
class Merger(object):
    ...
    # Use only if you want to copy paste from .txt files otherwise leave it to
    # 0 so numpy can read such files.
    prettyText = 1
    ...
    def __init__(self, paths, labels=None, output_path=None,
        ...
        # import the data from each path. Results in each of the paths has to
        # be consistent in terms of size
        self.means = []
        self.std_errs = []
        self.ybars = ybars  # Draw error bars?
        self.xbars = xbars
        # Only plot x error bars in this interval (eg every 5 datapoints)
        self.xbar_interval = 1
        self.colors = colors
        self.styles = styles
        self.markersize = markersize
        self.legend = legend
        # In case we want to use less than available number of samples
        self.maxSamples = maxSamples
        # Directories with samples less than this value will be ignored.
        self.minSamples = minSamples
        # Number of Samples for each experiments
        self.samples = []
        # By default assume all data has the same number of points along the X
        # axis.
        self.useLastDataPoint = False
        # Instead of mean of all experiments find the best performance among
        # them
        self.getMAX = getMAX
        self.output_path = output_path
        self.labels = labels
        self.showSplash = showSplash  # No figures just txt output
        # Number of string tokens to include between token_split_char
        # characters (eg '_') when producing legend
        self.num_label_suffixes = numLabelSuffixes
        # Character to use when splitting string token names
        self.token_split_char = token_split_char

        # Selectively add plotted items to legend
        self.legend_handles = []

        # Extract experiment paths by finding all subdirectories in all given
        # paths that contain experiment.
        ...
        if labels is None or labels == []:
            self.labels = [self.extractLabel(p, self.token_split_char)
                           for p in self.exp_paths]
        # print "Experiment Labels: ", self.labels

        self.exp_num = len(self.exp_paths)
        # Matrix with rows = metrics, cols = datapoints
        self.means = []
        self.std_errs = []
        self.time_95_mean = []
        ...
        # Number of datapoints to be shown for each graph (often this value is
        # 10 corresponding to 10 performance checks)
        self.datapoints_per_graph = []
        ...
        for expInd, exp in enumerate(self.exp_paths):
            means, std_errs, samples, time_95_mean, time_95_std_errs, dataPoints = self.parseExperiment(
                exp)
    ...
    def extractLabel(self, p, token_split_char):
        ...
        if firstTokenInd < 0:
            print 'ERROR: attempted to print more experiment labels than exist.'
        expName = ''
...
#             return tokens[-1]
    ...
    def parseExperiment(self, path):
        ...
        # This function also keeps track of the time it took each method to
        # reach its 95% of its final performance without dropping afterwards.
        if '/' in path:
        ...
        # Loop over all files to find max number of columns = x-axis datapoints
        # (performance checks)
        maxCols = 0
        for file in files:
...
# print '#### newvals ', masked_samples.std(axis=2) / sqrt(dataLengths)
        ...
        # Note that the stats method stats.sem ignores the mask, need
        # stats.mstats.
        if self.getMAX:
# return ma.max(masked_samples,axis=2),masked_samples.std(axis=2),
# samples_num, mean(times_95), std(times_95)/sqrt(samples_num), maxCols,
# batchMeans, batchStds
            return (
                ma.max(masked_samples, axis=2), stats.mstats.sem(masked_samples, axis=2), samples_num, mean(
                    times_95), std(times_95) / sqrt(samples_num), maxCols
            )
...
# return masked_samples.mean(axis=2),masked_samples.std(axis=2),
# samples_num, mean(times_95), std(times_95)/sqrt(samples_num), maxCols,
# batchMeans, batchStds
            return (
                meanVals, stdErrVals, samples_num, mean(
                    times_95), std(times_95) / sqrt(samples_num), maxCols
            )
    ...
    def showLast(self, Y_axis=None):
        ...
        if Y_axis is None:
            if self.ResultType == 'Policy Evaluation':
                Y_axis = 'Error'
            elif self.ResultType == 'Dynamic Means':
                Y_axis = 'L2 Observed Transition Errors'
            else:
                Y_axis = 'Return'
    ...
    def plot(self, Y_axis=None, X_axis=None, save=False, thousands=False):
        ...
        for i in arange(self.exp_num):
            # Look at parameters setting i, metric x_ind, for all datapoints
...
#            Y_Err = masked_Y_Err
            ...
            if not isOnCluster() and self.showSplash:
                if len(X) == 1 and self.ybars:
                    if self.xbars:
                        pl.errorbar(
                            X, Y, xerr=X_Err, yerr=Y_Err, marker=self.styles[
                                i %
                                len(self.styles)], linewidth=2, alpha=.7, color=self.colors[i %
                                                                                            len(self.colors)], markersize=self.markersize, label=self.labels[i])
                    else:
                        pl.errorbar(
                            X, Y, yerr=Y_Err, marker=self.styles[i %
                                                                 len(self.styles)], linewidth=2, alpha=.7, color=self.colors[i %
                                                                                                                             len(self.colors)], markersize=self.markersize, label=self.labels[i])
                    max_ = max(
                        max(Y + Y_Err),
                        max_)
                    min_ = min(min(Y - Y_Err),
                               min_)
                else:
                    if self.xbars:
                        ...
                        if self.xbar_interval > 1:
                            for j in arange(numBarsToPlot):
                                ...
                                X_Err[
                                    plottedInd +
                                    1:plottedInd +
                                    self.xbar_interval] = zeros(
                                    self.xbar_interval -
                                    1)
                        # Else we're plotting all error bars, make no changes
                        pl.errorbar(
                            X, Y, xerr=X_Err, linestyle='-', marker=self.styles[i % len(self.styles)], linewidth=2, alpha=.7, color=self.colors[i % len(self.colors)], ecolor='k',
                            markersize=self.markersize, label=self.labels[i])
                    else:
                        h = pl.plot(
                            X, Y, linestyle='-', marker=self.styles[i %
                                                                    len(self.styles)], linewidth=2, alpha=.7, color=self.colors[i %
                                                                                                                                len(self.colors)], markersize=self.markersize, label=self.labels[i])
...
#            print 'Y_Err',Y_Err
    ...
    def hasResults(self, path):
        return (
            len(glob.glob(os.path.join(path, '*-results.txt'))
                ) >= self.minSamples
        )
...
# print  M[return_row,target_col], last_return,
# abs(M[return_row,target_col]-last_return), abs(last_return)*.05
