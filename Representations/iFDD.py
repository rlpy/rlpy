...
class iFDD(Representation):
    ...
    maxRelevance            = - \
        np.inf  # Helper parameter to get a sense of appropriate threshold on the relevance for discovery
    ...
    def __init__(
        self, domain, logger, discovery_threshold, initial_representation,
        ...
        self.sortediFDDFeatures = Tools.PriorityQueueWithNovelty()
        ...
        if self.logger:
            self.logger.log("Initial Representation:\t%s" %
                            Tools.className(self.initial_representation))
            self.logger.log("Plus:\t\t\t%d" % self.iFDDPlus)
            self.logger.log("Sparsify:\t\t%d" % self.sparsify)
            self.logger.log("Cached:\t\t\t%d" % self.useCache)
            self.logger.log("Online Threshold:\t%0.3f" %
    ...
    def findFinalActiveFeatures(self, intialActiveFeatures):
        ...
        if 2 ** k <= self.features_num:
            ...
            if self.use_chirstoph_ordered_features:
                ...
                for candidate in Tools.powerset(initialSet, ascending=0):
                    if len(initialSet) == 0:
                        break  # No more initial features to be mapped to extended ones
    ...
    def updateWeight(self, p1_index, p2_index):
        if self.sparsify:
            newElem = np.atleast_2d(self.theta[
                                    p1_index] + self.theta[p2_index])
        else:
            newElem = None
        self.theta = Tools.addNewElementForAllActions(self.theta, 1, newElem)
        self.hashed_s = None  # We dont want to reuse the hased phi because phi function is changed!

    def addInitialFeatures(self):
        for i in np.arange(self.initial_representation.features_num):
    ...
    def batchDiscover(self, td_errors, phi, states):
        ...
        if self.iFDDPlus:
            relevances[non_zero_index] = Tools.divide(abs(relevances[non_zero_index]),
                                                      np.sqrt(counts[non_zero_index]))
            # Calculate relevances based on theoretical results of ICML 2013 potential submission
        else:
            relevances[non_zero_index] = relevances[
                non_zero_index]  # Based on Geramifard11_ICML Paper
        ...
        if SHOW_PLOT:
            ...
            e_vec = np.sort(e_vec)
            Tools.pl.ioff()
            Tools.pl.plot(e_vec, linewidth=3)
            Tools.pl.show()
        ...
        added_feature = False
        new_features = 0
        for j in np.arange(len(relevances)):
            ...
            if relevance > self.batchThreshold:
                ...
                if self.inspectPair(f1, f2, np.inf):
                    self.logger.log('New Feature %d: %s, Relevance = %0.3f' % (
    ...
    def __deepcopy__(self, memo):
        ifdd = iFDD(
            self.domain, self.logger, self.discovery_threshold, self.initial_representation, self.sparsify,
                    self.discretization, self.debug, self.useCache, self.maxBatchDicovery, self.batchThreshold, self.iFDDPlus)
...
class QiFDD(iFDD, QFunRepresentation):
    ...
    def updateWeight(self, p1_index, p2_index):
        ...
        if self.sparsify:
            newElem = (self.theta[p1_index::f] + self.theta[
                       p2_index::f]).reshape((-1, 1))
        else:
            newElem = None
        self.theta = Tools.addNewElementForAllActions(self.theta, a, newElem)
        self.hashed_s = None  # We dont want to reuse the hased phi because phi function is changed!
...
class iFDDK_potential(iFDD_potential):
    ...
    def update_statistics(self, rho, td_error, lambda_, gamma, phi, n_rho):
    ...
    def update_lazy_statistics(self, rho, td_error, lambda_, gamma, phi, y_a, y_b, t_rho, w, t, n_rho):
        # phi = phi_s[self.p1] * phi_s[self.p2]
        if lambda_ > 0:
...
class iFDDK(iFDD):
    ...
    def __init__(
        ...
        super(
            iFDDK, self).__init__(domain, logger, discovery_threshold, initial_representation,
    ...
    def post_discover(self, s, terminal, a, td_error, phi_s, rho=1):
        ...
        if not self.lazy:
            ...
            for f, potential in self.iFDD_potentials.items():
                potential.update_statistics(
                    rho, td_error, self.lambda_, self.gamma, dd[f], self.n_rho)
        ...
        if self.lambda_ > 0:
            self.y_a[self.n_rho] += np.exp(self.w) * (self.gamma * self.lambda_) ** (
                self.t - self.t_rho[self.n_rho]) * np.abs(td_error)
            self.y_b[self.n_rho] += np.exp(self.w) * (self.gamma * self.lambda_) ** (
    ...
    def inspectPair(self, g_index, h_index, td_error, phi_s, rho, plus):
        ...
        potential.update_lazy_statistics(
            rho, td_error, self.lambda_, self.gamma, phi, self.y_a,
