...
# show visualizations of the learning progress, e.g. value function
...
def make_experiment(id=1, path="./Results/Temp"):
    ...
    # Noise parameters used for some of the domains such as the GridWorld
    NOISE = .3
    BLOCKS = 6     # Number of blocks for the BlocksWorld domain
    # Representation ----------------------
    # CHANGE ME TO 20 # Number of bins used to discritize each continuous
    # dimension. Used for some representations, Suggestion: 30 for Acrobot, 20
    # for other domains
    DISCRITIZATION = 9
    # {'GridWorld':10, 'CartPole':20, 'BlocksWorld':100, 'SystemAdministrator':500, 'PST':500, 'InfCartPoleBalance': 20 } # Values used in tutorial RBF was 1000 though but it takes 13 hours time to run
    RBFS = 200
    iFDDOnlineThreshold = 1e7  # Edited by makexp.py script
    BatchDiscoveryThreshold = 0.05  # Edited by makexp.py script
    # BEBFNormThreshold           = #CONTROL:{'BlocksWorld':0.005,
    # 'InfCartPoleBalance':0.20}  # If the maximum norm of the td_errors is
    # less than this value, representation expansion halts until the next LSPI
    # iteration (if any).
    # Results will remain IDENTICAL, but often faster
    iFDD_CACHED = 1
    # Maximum Number of Features discovered on each iteration in the batch
    # mode of iFDD
    Max_Batch_Feature_Discovery = 1
    # {'BlocksWorld':0.0005,'InfCartPoleBalance':0.1} # See BEBF; essentially the region in which no penalty is applied for training
    BEBF_svm_epsilon = .1
    FourierOrder = 3     #
    # Should be on for online and off for batch methods. Sparsify the output
    # feature vectors at iFDD? [wont make a difference for 2 dimensional
    # spaces.
    iFDD_Sparsify = 1
    # True: relevance = abs(TD_Error)/norm(feature), False: relevance =
    # sum(abs(TD_error)) [ICML 11]
    iFDD_Plus = 1
    OMPTD_BAG_SIZE = 0
    # Policy ----------------------
    # EGreedy Often is .1 CHANGE ME if I am not .1<<<
    EPSILON = 0.1
    # Agent ----------------------
    # Boyan works better than dabney in some large domains such as pst. Decay
    # rate parameter; See Agent.py initialization for more information
    alpha_decay_mode = 'boyan'
    initial_alpha = .1  # Edited by makexp.py script
    boyan_N0 = 100000  # Edited by makexp.py script
    # In the Greedy_GQ Algorithm the second learning rate, Beta, is assumed to
    # be Alpha * THIS CONSTANT
    BetaCoef = 1e-6
    LAMBDA = 0.
    # Maximum Number of LSPI Iterations
    LSPI_iterations = 5 if not 'LSPI_iterations' in globals(
    ) else LSPI_iterations
    LSPI_windowSize = max_steps / performanceChecks
    # Minimum Weight Difference required to keep the LSPI loop going
    LSPI_WEIGHT_DIFF_TOL = 1e-3
    RE_LSPI_iterations = 50
    # Track the best policy through LSPI iterations using Monte-Carlo
    # simulation. It uses extra samples to evaluate the policy
    LSPI_return_best_policy = False
    # Represet large matrices such as A and all_phi with sparse matrices.
    LSPI_use_sparse = True

    # Policy Evaluation Parameters: ----------------------
    # Number of samples used for calculating the accuracy of a value function
    PolicyEvaluation_test_samples = 10000
    # Number of Monte-Carlo samples used to estimate Q(s,a) of the policy
    PolicyEvaluation_MC_samples = 100
    # Directory path to load the base of policy evaluations (True Q(s,a))
    PolicyEvaluation_LOAD_PATH = 'Policies/FixedPolicyEvaluations'

    # MDP Solver Parameters: ----------------------
    # Number of Samples used to estimate the expected r+\gamma*V(s') given a
    # state and action.
    NS_SAMPLES = 100
    # 2 Hours = Max Amount of time given to each planning algorithm (MDP
    # Solver) to think in seconds
    PLANNING_TIME = 60 * 60 * 2
    # Parameter used to define convergence in the planner
    ...
    # REPRESENTATION
    #================
    initial_rep = IndependentDiscretizationCompactBinary(
        domain,
        logger,
        discretization=DISCRITIZATION)
    ...
    representation = iFDD(
        ...
        iFDDOnlineThreshold,
        initial_rep,
        sparsify=iFDD_Sparsify,
        discretization=DISCRITIZATION,
        useCache=iFDD_CACHED,
        maxBatchDicovery=Max_Batch_Feature_Discovery,
        batchThreshold=BatchDiscoveryThreshold,
        iFDDPlus=iFDD_Plus)
    ...
    # policy          = FixedPolicy(representation,logger) # Use this together
    # with the PolicyEvaluation agent
    ...
    # agent           =
    # PolicyEvaluation(representation,policy,domain,logger,LEARNING_STEPS,
    # PolicyEvaluation_test_samples,PolicyEvaluation_MC_samples,PolicyEvaluation_LOAD_PATH,
    # re_iterations = RE_LSPI_iterations); PERFORMANCE_CHECKS  = 1 # Because
    # policy evaluation in one run, create the whole state matrix, having
    # multiple checks make the program confused.
