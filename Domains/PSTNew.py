...
class PST(Domain):
    ...
    P_NOM_FUEL_BURN     = 1.0  # Probability of nominal (1 unit) fuel burn on timestep [0.8 in tutorial]
...
#    CRASH_REWARD_COEFF  = -2.0 # Negative reward coefficient for running out of fuel (applied on every step) [C_crash] [-2.0 in tutorial]
    ...
    STOCH_FUEL_BURN     = None # 2 in Tutorial, set to NONE here to throw an error if future code changes involve this variable, not used in ICML.
...
#            [self.uav_sensor_vis[uav_id].remove() for uav_id in range(0,self.NUM_UAV)]
        ...
        for uav_id in range(0,self.NUM_UAV):
            ...
            uav_location = s[uav_ind + UAVIndex.LOC] # State index corresponding to the location of this uav
            uav_fuel = s[uav_ind + UAVIndex.FUEL]
            uav_sensor = s[uav_ind + UAVIndex.SENS_STATUS]
            uav_actuator = s[uav_ind + UAVIndex.ACT_STATUS]
...
#===============================================================================
    ...
    def step(self,s,a):
        ...
        nsStruct = copy.deepcopy(sStruct)
        # Subtract 1 below to give -1,0,1, easily sum actions
        actionVector = array(id2vec(a,self.LIMITS)) - 1 # returns list of form [0,1,0,2] corresponding to action of each uav
        nsStruct.locations += actionVector
...
#DEBUG        print 'action vector selected:',actionVector
        ...
        for uav_id in arange(0,self.NUM_UAV):            
            uav_ind = uav_id * UAVIndex.SIZE
            uav_location_ind = uav_ind + UAVIndex.LOC # State index corresponding to the location of this uav
...
#                continue
            ...
            uav_fuel_ind = uav_ind + UAVIndex.FUEL
            uav_sensor_ind = uav_ind + UAVIndex.SENS_STATUS
            uav_actuator_ind = uav_ind + UAVIndex.ACT_STATUS
...
#DEBUG                    print 'UAV',uav_id,'Failed sensor!'
            ...
            # No refuel/repair action taken for loitering in base, but fuel is burned
            else:
...
#debug        print totalStepReward,ns,self.isTerminal(ns)
    ...
    def s0(self):
        ...
        locations   = ones(self.NUM_UAV) * UAVLocation.MAINTENANCE
        fuel        = ones(self.NUM_UAV) * self.FULL_FUEL
        actuator    = ones(self.NUM_UAV * ActuatorState.RUNNING)
        sensor      = ones(self.NUM_UAV * SensorState.RUNNING)
    ...
    def state2Struct(self,s):
        ...
        fuelEndInd      = 2*NUM_UAV
        actuatorEndInd  = 3*NUM_UAV
        sensorEndInd    = 4*NUM_UAV
        locations = s[0         :NUM_UAV]
        fuel =      s[NUM_UAV   :fuelEndInd]
    ...
    def possibleActions(self,s):
        ...
        sStruct = self.state2Struct(s)
        for uav_id in range(0,self.NUM_UAV):
            if(sStruct.actuator(i) == ActuatorState.RUNNING or \
            sStruct.locations(i) == UAVLocation.REFUEL or \
            sStruct.locations(i) == UAVLocation.MAINTENANCE):
                uav_actions.append(UAVAction.LOITER)
                
            if(uav_fuel > 0): # This UAV is not crashed
                if(sStruct.locations(i) != UAVLocation.SURVEIL and \
                sStruct.actuator(i) == ActuatorState.RUNNING):
                    uav_actions.append(UAVAction.ADVANCE)
                    
                if(sStruct.locations(i) != UAVLocation.MAINTENANCE):
