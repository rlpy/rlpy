...
class RC_Com():
    ...
    def __init__(self, internal=False):
        self.state = CarState(
            pose=Pose(position=Point(0.00001, 0.00001, 0.00001)))
        self.XMAX = 3 / 2.0
        self.XMIN = -3 / 2.0
        self.YMAX = 2 / 2.0
        self.YMIN = -2 / 2.0
        if internal:
            subprocess.Popen(
                "roslaunch car_control control.launch veh:=RC num:=01 sim:=true",
                shell=True)
    ...
    def Step(self, s, a, visualize=True):
        ...
        try:
            ...
            quat_head = [
                ns.pose.orientation.x,
                ns.pose.orientation.y,
                ns.pose.orientation.z,
                ns.pose.orientation.w]
