...
class TrajectoryBasedValueIteration(MDPSolver):
    ...
    def solve(self):
        ...
        while deltaT(self.start_time) < self.planning_time and not converged:
            ...
            self.logger.log('PI #%d [%s]: BellmanUpdates=%d, ||Bellman_Error||=%0.4f, Return = %0.4f, Features = %d' % (iteration, hhmmss(deltaT(self.start_time)), bellmanUpdates, max_Bellman_Error, performance_return,self.representation.features_num))
