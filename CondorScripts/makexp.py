...
import argparse
parser = argparse.ArgumentParser(description='Generates grid-search files in the current directory from settings in the given script')
parser.add_argument("-f","--file", help="location of base script in your RLPy folder", type=str, default="main.py")
...
TOKEN_DICT={'T':['iFDDOnlineThreshold','BatchDiscoveryThreshold','discover_threshold'],
            'a':['initial_alpha'], 'N':['boyan_N0'],
            'k':['kernel_width'], 'd':['discretization'],
            's':['max_base_feat_sim'],
            'agent':['agent'], 'rep':['representation'],
            'domain':['domain'], 'solver':['MDPSolver'],
            'rbfs':['RBFS']}
for p,v in TOKEN_DICT.iteritems():
    parser.add_argument("-{}".format(p), help=" and ".join(v))
...
if __name__ == '__main__':
    ...
    mapping = {}   # Dictionary mapping each parameter name to its list of values
    parameters = [] #List of parameters in string
    counts = [] # Number of options for each parameter

    # extract list of values for each passed parameter
    for p, var in TOKEN_DICT.iteritems():
        if args.__dict__[p] is None:
            continue
        else:
            v = args.__dict__[p]
    ...
    if len(parameters) == 0:
        os.system('rm -rf main.py; cp {}/{} main.py'.format(RL_PYTHON_ROOT, args.file))
        print '>>>> Copied script file here.'
        sys.exit()
    ...
    for permutation in permutations:
        ...
        for param_index, value_index in enumerate(permutation):
            ...
            value = mapping[param][value_index]
        ...
        main_file_orig = open('{}/{}'.format(RL_PYTHON_ROOT, args.file), 'r')
        output_f =  open('%s/main.py' % dir_name, 'w')
        values = [mapping[parameters[i]][value_index] for i,value_index in enumerate(permutation)]
